...
# -------------------------------------------------------------------------
auth.settings.extra_fields['auth_user'] = [Field('Escuela'),Field('dni'),Field('iniciales')]
...
db.define_table('legajo',
   
    Field('foto', 'upload', default="", label=T('foto'),
                  autodelete=True,
...
db.define_table('n_medicas',
    ...
    Field('apellido',requires=IS_UPPER()),
    Field('nombre',requires=IS_UPPER()),
    Field('fecha','datetime',label="fecha de expedicion"),
    Field('diagnostico',requires=IS_UPPER()),
    Field('reposo',"integer",label="dias de reposo"), 
...
db.auth_user.Escuela.requires=IS_IN_SET(['Esc_de_cadetes','Super'])
